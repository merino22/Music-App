...
export class TopPanelComponent implements OnInit {
  ...
  formatTime(x: number)  {
    ...
    seconds.toString().length < 2 ? secondStr = `0${seconds}` : secondStr = seconds.toString();

    newTime = `${minutes}:${secondStr}`;
    return newTime;
  }
  ...
  sortTable(n: number) {
    ...
    table = document.getElementById("tablex") as HTMLTableElement;
    switching = true;
